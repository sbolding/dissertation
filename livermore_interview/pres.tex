...
\begin{itemize}
 ...
 \item[] We want to improve efficiency of MC calculations \\
...
%        \item[] For practical applications, spatial discretizations \\ must preserve the \colb{equilibrium diffusion limit} (EDL)
        \item[] Typically solved with implicit Monte Carlo (IMC) \\
            \colG{which linearizes the system over a time step}
\end{itemize}
\end{frame}
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
    \frametitle{Our HO system allows for \\ straight-forward variance reduction}
...
\begin{frame}
    \frametitle{Several specifics for most results shown are given below:}               
    {\addtolength\wideitemsep{11pt}
    \begin{itemize}
    \item Initial $\Delta t$ of $0.001$ sh (0.01 ns), \\ increased 15\% per step to 0.01 sh
    \item One HO solve per time step (two LO solves)
        \begin{itemize}
            \item \emph{each HO} solve has 3 ECMC batches \\ \colG{no mesh refinement}
        \end{itemize}
    \item FOM$=\frac{\displaystyle 1}{\displaystyle \|s^2\| N_{\text{total}}}$, \colG{normalized to IMC result}
    \end{itemize}
    }
\end{frame}
...
{\addtolength\leftmargini{-0.345in}
    \centering
        ...
        \begin{itemize}
            ...
            \item Different HO solvers: \colb{ECMC} (FOM=10,000), \\ \hspace{0.2in} \colr{ standard MC}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item[] Sample and track in time \\ \colG{Tally error projection at $t^{n+1}$}
        \vspace{0.08in}
...
\begin{frame}
    \frametitle{The time-closure parameters preserve \\ the accuracy of MC time integration}
    \fontsize{10.0pt}{10.0pt}\selectfont
    \vspace{0.1in}
    \begin{itemize}
        \item Material is near-void, so temperature uncouples  \\ \colG{take 3 large time
                steps and compare $T_{R}^{n+1}$}
            \item 300,000 histories/step
    \end{itemize}
    \begin{figure}
        \includegraphics[width=0.71\textwidth]{void_census_3steps.pdf}
    \end{figure}
\end{frame}

\begin{frame}
    \frametitle{A few other topics are under investigation}
    {\addtolength\wideitemsep{5pt}
        \begin{itemize}
            \item[] Negative intensities can occur in optically thick cells \\
            \colG{Investigating artificial source with alternate trial space}
        \item[] Using HO solution to estimate spatial closure \\
                \colG{by including face tallies}
            \item[] Source iteration with diffusion synthetic acceleration \\ \colG{for the
                LO equations}
        \end{itemize}}
\end{frame}
