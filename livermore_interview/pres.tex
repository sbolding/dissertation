...
\begin{frame}
    ...
    \item One HO solve per time step, with two LO solves
        ...
        \end{itemize}
    \item Figure of Merit: \boxed{\text{FOM}=\frac{\displaystyle 1}{\displaystyle \|\sigma^2\| N_{\text{total}}}}\\ \colG{are normalized to IMC results}
    \end{itemize}
    }
\end{frame}
...
\begin{frame}
    ...
    {\addtolength\wideitemsep{5pt}
        \begin{itemize}
            ...
            \colG{using an artificial source with alternate trial space}
        \item[] Using HO solution to estimate spatial closure \\
                \colG{by including face tallies in ECMC}
            \item[] Source iteration with diffusion synthetic acceleration \\ \colG{to solve the
                LO equation}
        \end{itemize}}
\end{frame}
