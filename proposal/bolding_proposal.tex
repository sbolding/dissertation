...
negativities in the LO equations, a modified spatial closure with a different relation between the
moments and edge values was used that is equivalent to the
standard FE lumping procedure.  This lumping-equivalent representation and S$_2$ equivalent consistency
...
fact that some terms are lagged, have led to the development of independent
approaches for the LO and HO systems thus far.

Typically, for a standard LDFE method,
the equations are lumped to produce a solution that is strongly resistant to negative
solutions (for 1D)~\cite{morel_newton}. However, standard FE lumping
...
dimensions, although it can produce convergence issues with the Newton method in very
diffusive problems.
...
modified the first space-angle moments.   Thus, the ECMC error estimates will rapidly stagnate,
...
added source method.  We will also investigate this source in conjuction with
a modified trial space obtained by linearly extrapolating the MC solution on an outflow face
through the average value of the solution. The next section explains how the solution on faces
is computed.
...
produces a negative solution.  The introduction of the discontinuous outflow, will also
affect the way fixups are applied to the linear interor solution of the LO equations.
This may improve convergence of the Newton method for the balance preserving fixup
...
Krylov solution~\cite{larson_morel_sn} of the LO equations to improve acceleration in such
cases.  Generally, Krylov
...
mixed optical thicknesses~\cite{larson_morel_sn}, which are typical in TRT
problems.
...
possible.  Inclusion of the time variable into the ECMC trial space needs to be
implemented and tested.  We also need to numerically demonstrate the
...
\begin{enumerate}
    \item The source iteration method with DSA for the LO solver will be applied to
        ...
        investigated.  The acceleration method will also be recast as a
        DSA-preconditioned Krylov method and tested.
