...
the outflow from a cell to be discontinuous in space, introducing extra unknowns.  For the HO solution, a discontinuous outflow for a space-angle cell should produce greater
...
face-based tally of the MC solution.  The error equation can be modified to estimate the outflow for a given batch.  For the LO equations, a parametric relation between the outflow and internal moments, as estimated by the lates HO solution,
will be used to eliminate the extra  unknowns. This will produce a consistent spatial closure between the HO and LO
solutions upon convergence.  The linear shape on the interior of the cell is still necessary due to the
temperature unknowns.  It is possible that poor statistics for the face
tallies may result in this trial space producing less acurate results compared to the standard LDFE solution, at least for fine meshes.  It is expected that this closure should improve accuracy in cells
where the in cells where the LDFE trial space does not produce a positive solution. 
