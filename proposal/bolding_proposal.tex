...
The goal of this modified transport problem is to maintain the zeroth moment of the solution, while
modifying the slope of the solution such that the ECMC solution is converging towards a
positive solution, at the rate the mesh resolution allows. If necessary, we can estimate a
...
solution becomes negative again.  We will also investigate this source in conjuction with
a modified trial space obtained by extrapolating the MC solution on an outflow face
through the interior of the solution. The next section explains how the solution on faces
is computed.
...
We hope to improve the efficiency of MC calculations in thin regions with ECMC while still
preserving the accuracy of a MC treatment in the time variable, e.g., error of the time trial space by 
including the time variable in the trial space for ECMC.  Thus, our $L$ operator is
now modified to include the time derivative, i.e.,
\begin{equation}
    LI = \frac{1}{c}\pderiv{I}{t} + \sigma_t I + \mu \pderiv{I}{x}
\end{equation}
The choice of trial space in time will also be investigated.  A doubly-discontinuous
trial space that is constant over the interior of the time step will be easiest to
implement. However, this trial space may produce problems in estimating the end of
...
histories reach the end of the time step.  If this proves to be too noise, a
linear-discontinuous treatment in time is an alternative solution.  The downside to this
approach is there is a projection error in the time.
This requires inversion of $L$ via MC to track the time of the particle and compute
averages of the particle.  It is necessary to close the equations in time.  All
consistency terms will exist as before, but now time-averaged quantities.  However we need
to eliminate the end of time step intensity in terms of time-averaged quantities,
incorporating the appropriate spatial discretization.  We will explore which form
of the closure is best by making some parametric combination of the known variables. This
closure forms LO equations that have the same difficulty to solve as the LO equations, but
have the potential, neglecting statistical noise, to preserve the accuracy in time of the
HO trial space.
...
\begin{enumerate}
    ...
    \item The inclusion of the time variable in the MC trial space with consistent
        closure of the LO equations remains to be implemented. We will test this in
        optically thin problems where the benefit of the time closure will be most
        apparent, as well as in thick problems where the closure needs to be stable enough
        to preserve the solution.  It may be necessary to implement some form of filtering
        on these closure parameters.
    \item Additional test problems will be simulated as necessary.
\end{enumerate}
\pagebreak
