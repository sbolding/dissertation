...
We perform a standard source iteration by lagging the scattering source in the LO
equations, which
...
preconditioner to a Krylov method for the acceleration to be effective~\cite{morel}.
%because DSA
...
\end{equation}
where the emission and scattering source are known from the previous LO solution and
$\tilde{I}^n$ is the LDFE projection of $I(x,\mu)$ from the previous time step.
 This defines a 
fixed-source, pure absorber transport problem. 
In operator notation, EQ.~\eqref{eq:ho_base} can be written as
\begin{equation}\label{te_oper}
    \B L I^{n+1} = q
\end{equation}
where $I^{n+1}$ is the transport solution of the end-of-time-step intensity.
...
Additionally, the SIMC work
explored alternate time discretizations for the temperature in the residual treatment, including a
a second-order accurate time treatment.  However, the second-order approximation
