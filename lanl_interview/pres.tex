...
\begin{frame}
    ...
    \addtolength{\leftmargini}{-1.2cm}
                    \begin{itemize}
                        \item[]Comparison of material temperatures for different $\Delta
                            t$
    \fontsize{10.2}{12.0}\selectfont
    \vspace{0.1in}
                \begin{itemize}
                    \item \colG{$\sigma_a \propto T^{-3}$, $c_v$ is small} 
                        \vspace{-0.1in}
            \item HOLO Newton Iterations required damping
        \end{itemize}
        \end{itemize}
            }
    \vspace{0.2in}
    ...
    {\setlength\unitlength{1mm}
    \begin{picture}(150mm,80mm)(0,0)
    \put(25,74){\underline{{IMC}}}
    \put(0,30){\centering\includegraphics[trim=0.0in 0.0in 0.0in
    0.30in,clip,width=0.485\textwidth]{mpv_mats_imc_nolegend.pdf}}
    \put(60,30){\centering\includegraphics[trim=0.0in 0.0in 0.0in
    0.24in,clip,width=0.485\textwidth]{mpv_mats_holo_nolegend.pdf}}
    \put(85,74){\underline{{HOLO}}}
    \put(25,25){\tikz{\draw[->,line width=0.02mm,Gray!90] (0,0) -- (-4mm,0);}}
...
\end{picture}}
    ...
    \begin{column}{0.5\textwidth}
        \vspace{0pt}
  \centering
    \includegraphics[width=\linewidth]{mpv_mats_imc_nolegend.pdf}
    \end{column}
    \begin{column}{0.5\textwidth}
        \vspace{0pt}
        \centering
    \includegraphics[width=\linewidth]{mpv_mats_holo_nolegend.pdf}
    \end{column}
\end{columns}
...
\begin{frame}
    ...
    \begin{itemize} 
        ...
        \item[]\pause \textbf{Alternatively}, add $\delta = -r^{(m)}_{\text{pos}}(x,\mu)$ to negative cells
            \\ \colG{$I_{\text{pos}}$ must be recomputing based on balance}
    \end{itemize}
}
\end{frame}
...
\begin{frame}
    \frametitle{A doubly-discontinuous trial space in time \\ allows for a HO
        temporal closure}
    \begin{columns}
    ...
    \vspace{0pt}
    \begin{itemize}
        \item Include $\frac{1}{c}\pderiv{ }{t}$ in $\B L$
        \item Must tally $\tilde I(x,\mu)$ at $t^{n+1}$ \\ \colG{similar to IMC census}
        \vspace{0.1in}
    \item In LO equations eliminate $\phi^{n+1}$ \\ using time-averaged quantities
