...
\sigma^2(P^{\textrm{mcnp}}_i)} 
	...
	\end{equation*} 
    \vspace{0.0in}
\item[] Equally weight $\chi^2$ values in a \colb{cost} function \\ \colG{the lower the
...
\begin{itemize}
    ...
    \item[] Scaling fission cross section 1.5\% ($<1 \sigma$) 
        ...
        \begin{itemize} \vspace{0.1in}
            \item Adjust \colb{elastic scattering} ($\sigma_{el}$) to compensate change in $\sigma_f$, for $\colb{E > 1 \text { keV}}$
