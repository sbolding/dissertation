...
\end{center}
\begin{itemize}
        \item Energy balance for angular radiation intensity $I(x,\mu,t)$ \\ and material
            temperature $T(x,t)$
        \item Equations are nonlinear and may be tightly coupled. \\  
            \colG{Opacities can be a strong function of $T$}
...
\begin{itemize}
    \item[] The system is \emph{linearized} over a time step $t\in[t^n,t^{n+1}]$ \\ 
        \colG{Opacities are evaluated at $t^n$}\vspace{0.1in}
...
Effective scattering can make MC very expensive  & MC solution has no scattering \\
Linearization can cause non-physical results & Fully implicit time-discretization and LO system resolves nonlinearities \\
...
\begin{column}{0.75\linewidth}
    ...
    \begin{enumerate}
      ...
      \phi_{i+1/2}^+ = 2\mom{\phi}_{R,i}^+ - \mom{\phi}_{L,i}^+$}}}}
            \end{center}
       \item \colG{Global system is solved with Newton's method, with lagged opacities}
       \end{enumerate}
    \end{column}
\end{columns}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item[] Each batch tallies the \colb{error} in current solution
            \begin{itemize}
                \item \colG{standard MC particle tracking}
            \item \colG{source can be negative}
    \end{itemize}

        \item[] Can reduce solution error \colb{globally} $\propto e^{-\alpha N}$ \\
            \colG{Adaptive $h$-refinement is required}
...
\begin{frame}
    \frametitle{We need a way to resolve issues when the LDFE intensity produces negative
    solutions}
    \begin{itemize} 
        \item Add a source to produce a positive intensity \\ \colG{which satisfies the
            residual equation} 
            \vspace{0.1in}
            \begin{tikzpicture}
                \hspace{-0.2in}
            \node at (0,0) {$\!\begin{aligned}
                \B L \tilde I^{(m)}  &= q - r^{(m)} \\
                \B L \tilde I_{\text{pos}}^{(m)} &= q - r^{(m)} + \tilde\delta^{(m)}
    \end{aligned}$};
             \node at (3,0) [fill=Gray!90,shape=single arrow,text width=0.7cm,text
             height=0.00001cm,line width=0.01cm] {};
             \node at (6,0) {${\displaystyle \tilde\delta^{m+1} = \B L \left(\tilde I^{(m)} - \tilde
             I_{\text{pos}}^{(m)}\right)}$}; 
            \end{tikzpicture}
        \item Can produce $\B L \tilde I_{\text{pos}}$ by scaling moments
            \\ \colG{currently we scale the slopes}
        \item Add source to LO equation \\ \colG{ensure energy balance is unchanged}
    \end{itemize}

\end{frame}

\begin{frame}
    \frametitle{A doubly-discontinous trial space \\ may increase accuracy}
    \begin{center}
        \begin{tikzpicture}[scale=0.62, every node/.style={transform shape}]
            \draw (1.0,4.0) node[fill,circle,inner sep=0pt,minimum
            size=4.2pt] {};
    ...
    \begin{itemize}
\item For HO solution, LD $\mu$ projection along the outgoing face
    \end{itemize}
