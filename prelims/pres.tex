...
\begin{frame}
    \frametitle{Implicit Monte Carlo (IMC) is the standard Monte Carlo transport method for TRT problems.}
        \vspace{-0.2in}
\begin{itemize}
    \item[] MAYBE ADD PIC OF PARTICLE TRACKING
...
\setlength\wideitemsep{0.2in}
            \item[] The \textbf{LO system} is space-angle moment equations,\\
              ...
              \item \textbf{Output:} a linear-discontinuous (LD) FE \\ 
                         \colb{scattering and emission source}
...
{\small
    \begin{minipage}[c]{0.9\linewidth}
        \vspace{24pt}
             \begin{equation*}
                 \tilde{I}_{ij}(x,\mu) = \ \ \tikzmark{moma}{\colb{I_{a,ij}}}\ \ + \frac{2}{h_x}\;\;\tikzmark{momx}{\colb{I_{x,ij}}}\ \ (x-x_i) +
                 \frac{2}{h_\mu} \ \ \tikzmark{mommu}{\colb{I_{\mu,ij}}}\ \ (\mu-\mu_i)
             \end{equation*}
        \begin{tikzpicture}[overlay, remember picture]
    \node (descr) at (4,2) {local volumetric tallies};
...
\begin{frame}
    ...
    latest residual equation}
            ...
            \node at (0,0) {$\!\begin{aligned}
                \B L \tilde I^{(m)}  &= q - r^{(m)} \\
                \B L \tilde I_{\text{pos}}^{(m)} &= q - r^{(m)} + \delta^{(m+1)}
            \end{aligned}$};
             \node at (2.58,0) [fill=Gray!50,shape=single arrow,text width=0.7cm,text
             height=0.00001cm,line width=0.01cm] {};
             \node at (5.2,0){$\!\boxed{\begin{aligned}
...
\begin{frame}
    \frametitle{We will use a linear doubly-discontinous (LDD) trial space \\ to allow for a HO
        spatial closure}
    \begin{columns}
    ...
    \vspace{0pt}
    \begin{itemize}
\item $\tilde I(x,\mu)$ will be linear in $\mu$ along face, \\
    \colG{estimated with MC face tallies}
    \end{itemize}
    \end{column}      
    \end{columns}
    \begin{itemize}
        \item In LO equations use LDD trial space for $\phi^{\pm}$\\
             \colG{The linear representations on interior will preserve EDL}
