...
radiation field and a high-temperature material.  The physics of TRT couples together the
transport of radiation through a material, with the absorption and emission of radiation
energy by a material.  Accurate modeling of TRT physics becomes relevant in the high-energy,
...
astrophysics phenomena.  In this work, we neglect
motion of the material, which would require solution methods appropriate for
radiation hydrodynamics~\cite{radhydro}, although our solution method is well suited to
couplling to material motion via typical operator-splitting methods for
radiation-hydrodynamic systems~\cite{radhydro}.
...
energy as a function of position.  The nonlinear
transport problem has high dimensionality and is computationally difficult to solve.
...
The governing equations for thermal radiative transfer problems are the radiation and material
 energy balance equations. The radiation transport equation is
 \begin{multline}
     ...
     \phi(\mathbf{r},\nu,t) \dd \Omega' \dd \nu' + \sigma_a(\nu,T)B_\nu(\mathbf{r},\nu,T).
 \end{multline}
 The material energy balance equation is
 \begin{multline}
     \rho c_v \pderiv{T(\mathbf{r},t)}{t} = \sigma_a(T) B(T)
 \end{multline}
Here, $B_\nu(\mathbf{r},\nu,T)$ is the black-body Planckian emission source~\cite{mihalis}, $\mathbf{r}$ is the position, $\mathbf{\Omega}$ is the direction
of particle travel; the macroscopic scattering, absoprtion, and total cross sections are $\sigma_s$,
$\sigma_s$, and $\sigma_t$, respectively. The integral of $B(\mathbf{r},\nu,T)$ over angle produces the 
grey Planckian emission source $\sigma_a(T) a c T^4$.

The absorption cross section $\sigma_a$ is typically a strong function of temperature,
i.e., $\sigma_a\equiv \sigma_a(T)$.

In derivation of the above equations, the emission source assumed to be described by the
Planck function at a point and the material is
well described by the local temperature, i.e., the conditions of local thermodynamic
equilibrium were assumed~\cite{mihalis}.
...
The purpose of the proposed research is to demonstrate the ability 
