...
discussed in Sec.~\ref{sec:ldfe_fixups} was used for cells where the solution for
...
Several different metrics are used quantify results throughout this chapter as needed.
...
\begin{equation} 
    S_i^2 =  \frac{1}{20-1} \sum_{l=1}^{20} \left(\overline{\phi_{i}} -
    \phi_{i}^l\right)^2,
\end{equation}
...
compared to a reference solution are computed for some simulations.
...
The sample mean of each of the above norms is estimated based on 20 independent
simulations; the sample standard deviation for each \emph{mean} is also reported, e.g.,
\begin{equation}
    s\left(\|e\|^{(l)}_{2,rel}\right) = \frac{1}{\sqrt{20}}\left[\frac{1}{20-1}\sum_{l=1}^{20} \left(
...
the fine mesh.  Cell-averaged quanties are found by spatially averaging the fine mesh solution over
