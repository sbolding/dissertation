...
\begin{equation}
    \|\phi^{HO} - \phi^{LO}\|^(l)_{2,rel} = \frac{\ds \int_0^X \left(
    \phi_{HO}^(l)(x) - \phi_{LO}^(l)(x) \right)^2 \dd x}{\ds 
       \int_0^X \left(\phi_{LO}(x)^(l)\right)^2 \dd x }
\end{equation}
where $\phi^{LO}(x)$ and $\phi^{HO}(x)$ are the LDFE representations in space of the
intensity from the HO and LO solvers.
The error between a reference solution and a fine solution for the $(l)$-th simulation
is computed as
\begin{equation}
    \|e\|^{(l)}_{2,rel} = \frac{\|\phi_{LO}^{n+1,(l)}(x) - \phi_{LO}^{ref}\|}{\|\phi_{LO}^{ref}\|}
\end{equation}
All L$_2$ norms are computed using quadrature over the finest spatial mesh.  A
integrated measure of the error in cell-averaged mean intensities on the coarse
mesh is computed as
\begin{equation}
    \|e\|^{(l)}_a = \left({\frac{\sum\limits_{i=1}^{N_c}
    \left(\phi_i^{n+1,(l)} - \phi_i^{n+1,ref}
\right)^2}{\sum\limits_{i=1}^{N_c}\left(\phi_i^{n+1,ref}\right)^2}}\right)^{1/2},
\end{equation}
where $\phi_i^{n+1,ref}$ is computed by spatially averaging the fine mesh solution over each
coarse spatial cell.

The sample mean of each quantity is estimated based on 20 independent
...
found that a third iteration did not increase consistency because of statistical
noise.  All simulations of this problem used 585,900 histories divided over 9 ECMC
...
Fig.~\ref{fig:smooth_compare} compares the solutions for various spatial closures at
coarse mesh sizes and a fine-mesh solution.  Visually there
is no difference in the results between the different spatial closures given by
Eq.~\eqref{} and Eq.~\eqref{}, as well as the LD closure. A step closure in all cells
