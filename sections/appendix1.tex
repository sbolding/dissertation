...
hat function moments (i.e., Eq.~\eqref{eq:x_moml} and~\eqref{eq:x_momr}) that is equivalent to the standard LDFE Galerkin method:
\begin{equation}
...
To eliminate the LO unknowns in a manner that produces the same moments as the lumped LDFE
...
eliminate the outflow from the equations.  However, the first moment equation does not
agree necessarily.
...
Based on these equations, iterations on the value of $T^*$ and unknowns $\phi^{n+1}$ and $T^{n+1}$ can be performed to converge the nonlinearities of the system. The algorithm for solving the LO equations, with iteration index
$m$, is defined as
\begin{enumerate}
...
\subsection{Spatially Varying Cross Sections}

To test the fixup we want to imitated the two-material problem in Sec.~\ref{sec:two},
