...
Solution of the diffusion equation provides a correction to $J$ and $\phi$ for the volumetric finite element
...
The opensource library \verb{mgmres{ was modified to implement the matrix-free version of
    ...
    the operation of $\left(\B I + \B D^{-1} \B S \right)\left(\B I  - \B M^{-1}\B S\right)$ applied to the Krylov vectors returned from the GMRES solve.
...
0.01 sh.  The larger time step size increases the amount of diffusive behavior.


\begin{table}
    \begin{tabular}{ccc} \hline
        Method & Iters./Time Step & Iters./Newton Step \\ \hline
        SI     &      &                 \\
        SI-DSA &      &       \\
        GMRES  &      &       \\
        GMRES-DSA &   &    \\ \hline
