...
of the system.  We have investigated using MC integration of the time derivative in the HO
...
\end{multline}
These equations are exact at this point.
...
intensity.  A modified implicit discretization can also be used
\begin{equation}\label{eq:tc_avg}
    I^{n+1} = \gamma \overline{I}
\end{equation}

A closure relation must be used to eliminate the unknowns at $t^{n+1}$ from each of the
equations, with the values from the previous time step taken as a known quantity.  Thus, it is necessary to have a closure relation for each moment and half
...
are computed with the trial-space representation of $I^{HO}(x,\mu,t)$ substituted into
Eq.~\eqref{eq:tc_diam} and Eq.~\eqref{eq:tc_avg}.
