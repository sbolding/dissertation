...
time is 0.003 sh.    For this problem, we expect IMC to be accurate
because with the small cross section the material energy equation becomes uncoupled.  
\begin{figure}[H]
  \centering
...
A comparison of the same results radiation temperatures is given in Fig.~\ref{fig:void_temp_compare}.  By plotting
...
would be negative particles that canceled out this error.  This effect is
significantly reduced when a smaller time step is taken, although it increases
the projection error between time steps. The time closure can lead to negative
...
step, averaged over 20 simulations.
...
the HOLO-TC method with the implicit-like and diamond-like closures, and with a BE discretization.  As
...
\subsection{Importance Sampling on Interior of the Time Step}
