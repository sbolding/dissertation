...
it increases the projection error between time steps.  For the case of a single batch, 
...
traveling past what the physical transport should allow.  The smearing near the foot of
...
Smaller time step sizes does increase the mesh imprinting.  However, it is important
to note that the mesh imprinting will be reduced as $\sigma_a$ is increased and
absorption-emission physics smooth out the angular intensity across a time step.
...
indicated in parenthesis next to the algorithm.  The results demonstrate that IMC is more
efficient than the ECMC method.  This is a limiting case; because minimal absorptions are
occuring, the IMC method is just advancing the initially sampled census particles
throughout time, so there is minimal sampling of the phase space.  The benefit of ECMC is
reduced comparitively due to the need to sample on the interior of the time step and the
truncation error in projecting onto the step trial space. At higher history counts, the
case of a single batch improves over standard IMC.
\begin{table}[H]
\centering
