...
\section{Resolving the LO solution in Optically Thick Cells}
\label{sec:negs}
...
unknowns equivalent to those from a lumped LD method in 1D.  The modified spatial closure, for positive
outflow, is
\begin{equation}
    \phi
\end{equation}
where a linear shape is still assumed.  This modified closure produces a linear
representation that preserves the zeroth moment of the solution, but has modified the
first spatial moment, i.e., the slope of the line.
