...
\begin{equation}\label{eq:stream_mom}
    ...
    \right]
\end{equation}
It is noted that this expression does not contain a cross section in the denomainator,
such as in the variable Eddington factor approach~\cite{ferguson}, thus this method will be
stable in a void.
...
the streaming term. At this point, the LO equations have too many unknowns: the relation between the volume
and face averaged quantities and the angular consistency parameters are not known a
priori. The HO solver and other approximations are used to close the system, eliminating
the extra unknowns from the equations. 
...
\begin{equation}\label{eq:ang_cons_vol}
    \cur{{\mu}}_{L,i}^{n+1,+} \simeq \frac{\mom{\mu
       \tilde I^{n+1,k+1/2}}_{L,i}^+}{\mom{\tilde I^{n+1,k+1/2}}_{L,i}^+} =  \frac{
{\displaystyle \frac{2}{h_i}} \int\limits_0^1 \int\limits_\xl^\xr \mu \, b_{L,i}(x)
...
on the LDFE functional representation $\tilde I(x,\mu)$ provided by the HO solution.

\subsection{LDFE Spatial Closure}

The relation between the volume and face averaged quantities must be known to eliminate
the final auxiliary unknowns.
To close the LO system spatially, we will reduce the equation to only the desired moment
...
Sec.~\ref{sec:spat_clos}.  Because there are no derivatives of $T$ in Eq.~\eqref{lo_mat}, there is no need
...
positive flow (i.e., Eq.~\eqref{eq:exact_lmomp} and~\eqref{eq:exact_rmomp}).  The face terms $\mu_{i-1/2}$ and $\phi_{i-1/2}$
...
moments.  The linear approximation $\phi^+(x)=b_{L,i}(x)\phi_{L,i} +
b_{R,i}(x)\phi_{R,i}$, for $x\in(\xl,\xr]$, is substituted into the definition for
the moments (i.e., Eq.~\eqref{eq:x_moml}~and~\eqref{eq:x_momr}) and solved for the LD edge value
$\phi_{i,R}$.  The resulting outflow relation for positive flow is $\phi^+_{i+1/2} = 2\mom{\phi}_R^+ -
\mom{\phi}_L^+$. The LD closure, with upwinding, for the $L$ equation and positive half-range is
\begin{multline}
...
equations.  The LD closure for the negative half range gives $\phi^-_{i-1/2} = 2\mom{\phi}_L^- -
\mom{\phi_R}^+$.   These equations are equivalent in numerical complexity to an LDFE
...
positive, the outflow is positive. Details on the
