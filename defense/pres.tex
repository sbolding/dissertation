...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{frame}
 \frametitle{ECMC uses a \colb{projection} $\tilde I(x,\mu)$ onto a \\ space-angle LDFE  mesh to represent the solution}
     \begin{itemize}
         \item[] 
{\small
...
\begin{frame}
    \frametitle{We eliminate auxillary spatial unknowns  \\ to close global
        system of equations
    }
    \begin{columns}
    ...
    \begin{centering}
        \begin{tikzpicture}[scale=0.62, every node/.style={transform shape}]
            \filldraw[color=black,fill=white] (1.0,2.125) circle (2.1pt);
            \draw (1.0,0.4) -- (1.0,0.6) node[below, pos=0.4] {$x_{i-1/2}$};
            ...
            \filldraw[color=black,fill=white] (5.9,3.7) circle (2.1pt);
        \end{tikzpicture}
        \vspace{0.1in}
        \begin{tikzpicture}[scale=0.58, every node/.style={transform shape}]
            \hspace{-0.13in}
...
\begin{column}{0.75\linewidth}
 ...
 %       \item \colG{Energy is always conserved}
       \end{enumerate}
   }
    \end{column}
\end{columns}
    ...
    \begin{itemize}
        \item[] Used source iteration (SI) with WLA-DSA  \\ \colG{for (effective) scattering source
            ...
            \begin{enumerate}
                ...
                \item Update moments w/ approx. local balance equations 
...
\begin{frame}
    ...
    \begin{itemize}
        \item HOLO method is written in stand-alone \textrm{C}\texttt{++} (15k lines) \\
        \colG{IMC results from Jayenne (LANL code)}
    \item Unless stated, one HO solve per time step, \\  with two LO solves
        \begin{itemize}
    ...
    \item Figure of Merit:  \boxed{\text{FOM}=\frac{\displaystyle 1}{\displaystyle
        \left(\frac{\ds\|\sigma(\phi_i)\|}{\ds\|\phi_i\|}\right)^2 N_{\text{total}}}}\\\vspace{0.05in} \colG{normalized so IMC FOM=1}
    \end{itemize}
    }
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item Fairly diffusive domain so that \\ linear discontinuous $T(x)$ is positive
            \begin{itemize}
                \item[]\colG{ $\Delta t$ increased from 0.001 sh to  $\Delta t_{\max} = 0.01$ sh}
            \end{itemize}
...
\colG{*$1\sigma \leq 0.01 \%$ for all results}
    ...
    \begin{itemize}
            \pause
    \item For ECMC, $\tilde I_{HO}(x,\mu)$ does \emph{not satisfy} moment equations
            \vspace{-0.1in}
            \begin{equation*}
...
\vspace{0.2in}
    \begin{figure}
        \vspace{-0.1in} \includegraphics[width=0.56099\textwidth]{neut_analytic_phi.pdf}
    \centering
    \end{figure}
\end{frame}

\begin{frame}
    \frametitle{The angular intensity is difficult to resolve \\ at the material interface and near $\mu=0$}
    \begin{columns}
    \begin{column}{0.5\textwidth}
        \vspace{0pt}
  \centering
...
\begin{frame}
    \frametitle{The time variable is included in the ECMC trial space \\ allowing for
        accurate time integration of HO intensity }
    \begin{columns}
...
\fontsize{10.0pt}{9.0pt}\selectfont
    \begin{itemize}
        \item[] Introduce  trial space representation for $I(x,\mu,t)$ \\ 
            \colG{step doubly discontinuous in $t$, LDFE in $x$ \& $\mu$}
        \item[] Include continuous $\frac{1}{c}\pderiv{}{t}\left(\cdot\right)$ in $\B L$ for residual
            source\\ \colG{leaving $T(x)$ and $T^4(x)$ implicit}:
            \vspace{-0.1in}
            \begin{equation*}
        ...
        \item[] Sample and track particle histories in time. \\ \colG{Tally time-averaged and
                $t^{n+1}$ errors}
        \end{itemize}
    }
    \end{column}      
...
\item Eliminate space-angle moments of $I^{n+1}_{LO}$  in terms of
    \textbf{time-averaged} moments $\overline I_{LO}$, e.g.,
            \begin{equation*}
...
\begin{frame}
    ...
    {   \fontsize{9.0pt}{9.0pt}\selectfont
        \item Comparison radiation energy densities $E_{r} = \phi(x)/c$ \\ for
            \colb{time-averaged} and \colb{census} values
            \vspace{-0.1in}
        \item 3 batches of 100,000 hists./step, 100 $x$ cells, \textbf{FOM=0.53}}
    \end{itemize}
    \end{itemize}
\begin{figure}[H]
  \centering
