...
\begin{frame}
    \frametitle{We eliminate auxillary spatial unknowns \\ with a linear-discontinuous (LD) representation
    }
    \begin{columns}
...
\begin{column}{0.75\linewidth}
    ...
    \begin{enumerate}
        ...
        \item Assume $\phi^{\pm}$ linear on interior
        \item Eliminate outflows with parametric closure from HO solution, e.g., \vspace{0.1in}
...
\begin{frame}
    ...
    \begin{itemize}
        \item HOLO method is stand-alone C++ code (15k lines) \\
    ...
    \item One HO solve per time step, with two LO solves
        ...
        \end{itemize}
    \item Lumped LD discretization for $T(x)$ and $\phi(x)$
...
\begin{frame}
    ...
    \begin{itemize}
\item Compute error for 50 cells, compared to 500 cell reference \\ \colG{Average results from 20 independent simulations}
    \end{itemize}
        \begin{table}
            \begin{center}
    \begin{tabular}{r|ccc} \hline
        Spatial Closure & \multicolumn{1}{|c}{$\|e(x)\|_{rel}$}  &
